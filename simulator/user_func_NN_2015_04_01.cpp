...
{
	/*qDebug() << "umain";
	qDebug() << "encData" << InData->encData[0] << InData->encData[1] << InData->encData[2];
	qDebug() << "vDelta_o" << vDelta_o[0] << vDelta_o[1] << vDelta_o[2] << vDelta_o[3] << vDelta_o[4] << vDelta_o[5];
	qDebug() << "vPHIo" << vPHIo[0] << vPHIo[1] << vPHIo[2];
	qDebug() << "vStateVect" << vStateVect[0] << vStateVect[1] << vStateVect[2] << vStateVect[3] << vStateVect[4] << vStateVect[5];*/
	...
	float m4[508];
	float m5[508];
	float dot_gamma, dot_psi, dot_theta;
    float Gamma_coef = -10;
	int i;
	...
	// Производные углов
	dot_gamma = (vStateVect [2] - vStateVectPrev [2])/dt; 
	dot_psi = (vStateVect [1] - vStateVectPrev [1])/dt; 
	dot_theta = (vStateVect [0] - vStateVectPrev [0])/dt; 
	// Вычисление угловой скорости по энкодерам
    vStateVect [3]=dot_gamma+dot_psi*cosf(vStateVect [0]); // omegax
    vStateVect [4]=dot_theta*sinf(vStateVect [2])-dot_psi*sinf(vStateVect [0])*cosf(vStateVect [2]); // omegay
    vStateVect [5]=dot_theta*cosf(vStateVect [2])+dot_psi*sinf(vStateVect [0])*sinf(vStateVect [2]); // omegaz
    ...
    for (i=0;i<3;i++) vErrorVect[i] = vPHIo[i] - vStateVect[i];
    ...
    // Интегрирование весов НС методом Эйлера
