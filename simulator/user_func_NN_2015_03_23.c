...
int umain(const _DLMIN *InData, _DLMOUT *OutData)
{
	...
	// Диагональная матрица Гамма, имеющая размер iNumbNNЧ Х iNumbNN, на диагонали расположен элемент Gamma_coef
	Diag_matrix( iNumbNN, Gamma_coef, (float*) mGamma );
  	  	  	
  	// Подготовка входного вектора по данным энкодеров
	for (i=0;i<3;i++) {
...
void fRBF (const float *vStateVect, float *vRBF){
	...
	*vRBF = 1;
	...
	for (i=-10;i<10;i++){
		//k++;
		//vRBF ++;
		//*vRBF=expf(-(vStateVect[0]-vDelta[0]*i)*(vStateVect[0]-vDelta[0])/vSigma[0]);
		vRBF[i+11]=expf(-(vStateVect[0]-vDelta[0]*i)*(vStateVect[0]-vDelta[0])/vSigma[0]);
		vRBF[i+31]=expf(-(vStateVect[1]-vDelta[1]*i)*(vStateVect[1]-vDelta[1])/vSigma[1]);
		vRBF[i+51]=expf(-(vStateVect[2]-vDelta[2]*i)*(vStateVect[2]-vDelta[2])/vSigma[2]);
		vRBF[i+71]=expf(-(vStateVect[3]-vDelta[3]*i)*(vStateVect[3]-vDelta[3])/vSigma[3]);
		vRBF[i+91]=expf(-(vStateVect[4]-vDelta[4]*i)*(vStateVect[4]-vDelta[4])/vSigma[4]);
		vRBF[i+111]=expf(-(vStateVect[5]-vDelta[5]*i)*(vStateVect[5]-vDelta[5])/vSigma[5]);
	}
}
/*
